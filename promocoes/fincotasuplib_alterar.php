...
include_once('../database/fincotasuplib.php');
...
$supervisor = buscaSupervisor($fcccod, $supcod, $DtIVig);
//echo json_encode($id_recid);
...
<body>
    ...
    <div class="container-fluid">
        ...
        <form method="post" id="alterarSupervisorForm">
            <div class="row">
                <div class="col-2">
                    ...
                    <input type="hidden" class="form-control ts-input" name="fcccod" value="<?php echo $supervisor[0]['fcccod'] ?>" readonly>
                </div>
                <div class="col">
                    <label class="form-label ts-label">Nome</label>
                    <input type="text" class="form-control ts-input" value="<?php echo $supervisor[0]['supnom'] ?>" readonly>
                </div>
                <div class="col">
                    <label class="form-label ts-label">Inicio</label>
                    <input type="date" class="form-control ts-input" name="DtIVig" value="<?php echo $supervisor[0]['DtIVig'] ?>" readonly>
                </div>
                <div class="col">
                    <label class="form-label ts-label">Final</label>
                    <input type="date" class="form-control ts-input" name="DtFVig" value="<?php echo $supervisor[0]['DtFVig'] ?>">
                </div>
                <div class="col">
                    <label class="form-label ts-label">Cotas Liberadas</label>
                    <input type="text" class="form-control ts-input" name="CotasLib" value="<?php echo $supervisor[0]['CotasLib'] ?>">
                </div>
            </div>
    ...
    <script>
        ...
        $.ajax({
            type: 'POST',
            dataType: 'html',
            url: '../database/fincotasup.php?operacao=buscar',
            beforeSend: function() {
        ...
        $("#alterarSupervisorForm").submit(function(event) {
            event.preventDefault();
            var formData = new FormData(this);
            $.ajax({
                url: "../database/fincotasuplib.php?operacao=alterar",
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
